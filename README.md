...
DXF içe aktarılırken her parça 1° adımlarla 0–359° arasında ön taranır; en küçük bounding-box alanını veren açı parçanın başlangıç açısı kabul edilir. Kullanıcının seçtiği dönüş seçenekleri bu optimize edilmiş başlangıç açısına göre uygulanır. V1 yerleşim motoru, gerçek dış konturlar arasında poligon kesişimi ve minimum kenar mesafesi kontrolü kullanır. Bounding-box yalnızca hızlı ön eleme, sıralama ve raporlama amacıyla kullanılır. Böylece tamamlayıcı üçgenler gibi parçalar aynı dikdörtgen alan içerisinde birbirine geçebilir. Motor tek bir greedy geçiş yerine alan, yükseklik, genişlik, uzun kenar ve en-boy oranı önceliklerinden türetilen çoklu başlangıçları karşılaştırır; farklı dönüş tercihlerini dener ve seçilen sonucu yerleşim sonrası tekrar sıkıştırır. Global skor, hizalama modlarında karşı kenarda kalan başıboş parçaları ve en yakın komşusundan kopuk duran parçaları güçlü biçimde cezalandırır. Alt hizalama sıkıştırması, izin verilen dönüşler arasından ince-uzun parçaların yatay varyantlarını da yeniden dener; farklı parçaların kenar koordinatlarını çaprazlayarak alt bölgedeki kullanılabilir boşlukları arar ve toplam yerleşim yüksekliğini öncelikli olarak düşürür. Tüm parçaların yerleşmesi mutlak birincil hedeftir; tamamlanmamış sonuçların görsel skoru ne kadar iyi olursa olsun daha fazla parça yerleştiren sonuç seçilir. İlk aramada dışarıda kalan parçalar sonraki başlangıçlarda sıranın başına alınır. Yerleşmeyen parçalar için doğrudan yeniden ekleme ve gerektiğinde tek yerleşmiş parçayı çıkarıp ikisini yeniden yerleştiren kurtarma geçişi uygulanır. Skor; normalize edilmiş kullanılan bounding alanı, seçili kenara uzaklık, parçalanma yaklaşımı ve en büyük artık şeridini birlikte değerlendirir. Aday koordinatlarına parça boşluğu doğrudan eklenir; böylece boşluk pozitifken kenar adaylarının yanlışlıkla çakışmalı sayılması önlenir. “En büyük artık alan” modu parçaları bir tarafta kompakt tutarak büyük tek parça artığı teşvik eder.
...
3. Stok, margin, parça boşluğu, hizalama ve döndürme seçeneklerini belirleyin. Varsayılan gelişmiş ağırlıklar normalize edilmiş kalite senaryoları için `alan=1.2`, `kenar=1.4`, `artık=0.8`, `parçalanma=0.35` olarak dengelenmiştir.
...
## Gerçek DXF regresyon testi

`node tests/real-dxf-set.js`, `tests/` altında bulunan tüm `.dxf` dosyalarını otomatik keşfeder, ayrıştırır ve tüm parçaların geçerli biçimde 60 saniye içinde yerleştiğini doğrular. Depoda DXF bulunmadığında test açık bir `SKIP` mesajı verir.
